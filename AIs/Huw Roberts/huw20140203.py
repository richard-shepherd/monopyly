...
class HuwAI20140203(PlayerAIBase):
    '''
    ...
    def property_offered_for_auction(self, game_state, me, property):
        ...
        if (info.state == SetInfo.State.TWO_OF_THREE_OWNED_SAME or info.State == SetInfo.State.ONE_OF_TWO_OWNED) \
            ...
            return self.make_offer_amount(me, property.price * 2.5)
        if info.state == SetInfo.State.TWO_OF_THREE_OWNED_SAME or info.State == SetInfo.State.ONE_OF_TWO_OWNED:
            return self.make_offer_amount(me, property.price * 2)
        if info.state == SetInfo.State.ONE_OF_THREE_OWNED and info.owners[0] == me:
            return self.make_offer_amount(me, property.price * 1.5)
        return self.make_offer_amount(me, property.price)
