...
class RatbertAI(PlayerAIBase):
    ...
    def check_start_of_game(self, game_state):
        ...
        for p in game_state.players:
            if (not p.name in self.others):
                ...
                self.others[p.name] = OtherPlayer(p)
    ...
    def dispose_of_property(self, game_state, me):
        player = None
        wins = 1000000
        for p in game_state.players:
            if self.others_in_game[p.name].wins < wins and p.name != me.name:
                player = p
        if player is None:
            return None

        return DealProposal(player, [p for p in me.state.properties], None, 0, 1)


    def propose_deal(self, game_state, me):
        Logger.log("* Propose deal perhaps", Logger.DEBUG)
        if self.going_bankrupt:
            return self.dispose_of_property(game_state, me)
    ...
    def money_will_be_taken(self, player, amount):
        ...
        if self.to_sell > self.hard_reserve:
            self.going_bankrupt = True
            Logger.log("Going bankrupt")

    def player_went_bankrupt(self, player):
        if player.ai == self:
            Logger.log("Went bankrupt")
    ...
    def sell_houses(self, game_state, me):
        Logger.log("* Sell perhaps", Logger.DEBUG)
        if self.to_sell > 0:
            to_sell = self.to_sell
            self.to_sell = 0
            return do_sell_houses(game_state, me, self.sets_we_like, to_sell)
    ...
    def game_over(self, winner, maximum_rounds_played):
        self.others[winner.name].wins += 1
