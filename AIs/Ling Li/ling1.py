...
class Ling1AI(PlayerAIBase):
    ...
    def landed_on_unowned_property(self, game_state, player, property):
        ...
        if player.net_worth > (self.cash_reserve + property.price):
    ...
    def propose_deal(self, game_state, player):
        for property_name in self.properties_we_like:
            ...
            if player.state.cash - self.cash_reserve > price_offered:
    ...
    def property_offered_for_auction(self, game_state, player, property):
        ...
        if self.cash_reserve > property.price + 1:
            return property.price + 1
        return self.cash_reserve / 2
    ...
    def mortgage_properties(self, game_state, player):
        if player.state.cash < self.cash_reserve or player.state.cash - self.money_to_be_taken < self.cash_reserve:
            ...
            for k in sorted(properties_to_mortgage.keys()):
                ...
                if player.state.cash + mortgage_total - self.money_to_be_taken >= self.cash_reserve:
                    break
    ...
    def unmortgage_properties(self, game_state, player):
        ...
        current_cash = player.state.cash - self.cash_reserve
